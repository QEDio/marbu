$(function(){
  function add_remove () {
    ...
    el.find('.add-remove-element:last')
      .after('<button type="button" class="add" title="Add row">Add</button>');
  ...
  var updateButtonsAndAnotherthings;

  $("input, select, textarea").focus(function(event) {
    if (!$(event.target).data("initialValue")) {
      $(event.target).data("initialValue", this.value);
    }
  }).blur(function() {
    setTimeout(updateButtonsAndAnotherthings, 10);
  });
  ...
  function sample_data_click (e) {
    ...
    if (!input || !el.data('content'))
      return false;

    while (el.data('content')) {
      keys.push(el.data('content'));
      el = el.parents('.ui-collapsible:first');
    }

    var value = keys.reverse().join('.');
  ...
  updateButtonsAndAnotherthings = function() {
    var dirty, field, _i, _len, _ref;
    dirty = false;
    _ref = $("input, select, textarea");
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      field = _ref[_i];
      field = $(field);
      if (field.data("initialValue") && field.data("initialValue") !== field.val()) {
        dirty = true;
        break;
      }
    }
